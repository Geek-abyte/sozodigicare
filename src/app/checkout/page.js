...
export default function Checkout() {
  ...
  const pendingPrescription = cartItems.some(
    (item) =>
      ...
      item.prescriptionLinkStatus === "pending",
  );

  const rejectedPrescription = cartItems.some(
    (item) =>
      item.product.prescriptionRequired &&
      item.prescriptionLinkStatus === "rejected",
  );
    ...
    <div className="max-w-5xl mx-auto px-4 py-8 lg:py-12">
      ...
      {!allApproved ? (
        <div className="bg-yellow-100 text-yellow-800 p-4 rounded-lg shadow-md mb-8">
          {pendingPrescription && (
            <p className="mb-2">
              🔄 Some prescription items are <strong>pending approval</strong>.
              Please wait until they are approved.
            </p>
          )}
          {rejectedPrescription && (
            <p className="mb-2">
              ❌ Some prescription items were <strong>rejected</strong>. Please
              link a valid prescription to proceed.
            </p>
          )}
          <p className="mt-2">
            If your prescription was rejected, you can re-upload or link another
            prescription from your cart.
          </p>
        </div>
      ) : (
        <>
