...
const SessionPage = () => {
  ...
  useEffect(() => {
    if (
      appointmentRef.current?.session?.appointment?.status === "completed" &&
      !appointmentRef.current?.session?.feedback
    ) {
  ...
  const sessionEnded =
    appointment?.session?.appointment?.status === "completed";

  if (sessionEnded && session?.user?.role === "user") {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] mt-4 px-6 py-10 bg-white dark:bg-gray-800 rounded-xl shadow-xl text-center transition-all">
        ...
        <p className="text-gray-600 dark:text-gray-300 mb-6 max-w-md">
          Thank you for attending the consultation. Please rate your experience
          or book a follow-up session.
        </p>
            ...
            <ul className="list-disc list-inside text-sm">
              {appointment.session.prescriptions?.length > 0 && (
                <li>
                  ...
                  <Link
                    href="/admin/doctor-prescriptions"
