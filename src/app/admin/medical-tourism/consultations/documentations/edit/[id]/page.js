...
const EditConsultationDocumentation = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      if (docRes && docRes._id) {
        setForm({
          appointment: docRes.appointment._id || "",
          notes: docRes.notes || "",
          document: null,
        });
      }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.appointment) {
      addToast("Appointment is required", "error");
      return;
    }
  ...
  return (
    <div className="max-w-3xl mx-auto bg-white dark:bg-gray-900 rounded-xl shadow p-6">
      <h1 className="text-2xl font-semibold text-gray-800 dark:text-white mb-6">
        Edit Consultation Documentation
      </h1>
...
export default EditConsultationDocumentation;
